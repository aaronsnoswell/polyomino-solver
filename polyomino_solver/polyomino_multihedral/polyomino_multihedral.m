...
%  Parameters:
%
%    Input, integer R_SHAPE(M_R,N_R), the binary matrix describing the region.
%
%    Input, integer P_NUM, the number of polyomino shapes to be used.
%
%    Input, integer P_SHAPES(M_R,N_R,P_NUM), a binary matrix describing the 
...
%
%    Input, integer D(P_NUM), tells us how many copies of each polyomino
%    we must use in the tiling.
%
%    Input, string FILENAME, a name for the LP file that will contain the
%    definition of the problem.
%
%    Input, string COMMENT, a comment that will be the first line of the 
%    LP file.
%
%    Output, integer M, N, X_NUM, the number of equations, the number of 
%    variables, and the number of binary solutions found.
%
%    Output, integer A(M,N), the matrix of equation coefficients, which should
%    all be 0 or 1.
%
%    Output, integer B(M), the right hand sides, which should all be 1.
%
%    Output, integer X(N,X_NUM), a set of X_NUM solutions to the problem, each
...
%
  for k = 1 : p_num
    ...
    if ( ~ i4mat_is_binary ( m_p, n_p, p ) )
      ...
      fprintf ( 1, '  The matrix P(%d) has entries that are not 0 and not 1.\n', k );
